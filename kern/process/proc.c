...
static int load_icode(int fd, int argc, char **kargv) {
    ...
    off_t phoff = (off_t)elf->e_phoff;
    for (int i = 0; i < elf->e_phnum; i++) {
        //(3.2) read raw data content in file and resolve proghdr based on info in elfhdr
        if ((ret = load_icode_read(fd, ph, sizeof(struct proghdr), phoff)) != 0) {
            goto bad_elf_cleanup_pgdir;
        }
        phoff += sizeof(struct proghdr);
    ...
    //(6) setup uargc and uargv in user stacks
    // 参数 argc 和 argv 在栈上的位置, 和 user/libs/initcode.S 中读取处对应
    // argc: ESP, argv[0]: ESP + 0x4, argv[1]: ESP + 0x8, ...
    uintptr_t stacktop = USTACKTOP;
    for (int i = 0; i < argc; i++) {
        stacktop -= strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }
    stacktop = ROUNDDOWN(stacktop, sizeof(long));
    char *uargv_content = (char *)stacktop;
    stacktop -= argc * sizeof(char *);
    char **uargv = (char **)stacktop;
    stacktop -= sizeof(int);
    *(int *)stacktop = argc;
    for (int i = 0; i < argc; i++) {
        uargv[i] = strcpy(uargv_content, kargv[i]);
        uargv_content += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }
...
static int kernel_execve(const char *name, const char **argv) {
    ...
    asm volatile("int %1;" : "=a"(ret) : "i"(T_SYSCALL), "0"(SYS_exec), "d"(name), "c"(argc), "b"(argv) : "memory");
    return ret;
}
